...
- **⚠️ UPSERT + KOLON YETKİSİ TUZAĞI (2026-08-17'de yaşandı).** Profil kaydetme `permission denied for table profiles` veriyordu. Kök neden: istemci `.upsert({ id, ... })` kullanıyor; PostgREST bunu `insert ... on conflict (id) do update set id = excluded.id, ...` haline getiriyor, yani **`id` de UPDATE listesine giriyor.** `id`ye yalnızca INSERT yetkisi verilmişti. Düzeltme: `20260817120000_fix_profiles_update_grant.sql` → `grant update (id)`.
  - **TEŞHİS KURALI — mesajın biçimine bak:** `permission denied for table X` = **grant/kolon yetkisi** eksik. `new row violates row-level security policy` = **policy** hatası. İkisi tamamen farklı yerlerde aranır; bu ayrım teşhis süresini kısaltır.
  - Kolon bazlı yetki kullanılan tablolarda `upsert`, `update` grant listesinin **birincil anahtarı da içermesini** gerektirir.
- **⚠️ SÜRÜM UYUŞMAZLIĞI — genel kural (bu hatanın sebebi DEĞİLDİ, ama gerçek bir risk).** RLS'i sıkılaştırmak daha önce yayınlanmış istemcileri kırar. 2026-08-17'de TestFlight'taki iOS 1.0.7/#11 gerçekten RLS kod değişikliklerinden önceki commit'ten (`34de6f4`) alınmıştı ve `profiles.select('*')` yüzünden kırıktı; 1.0.8/#12 ile düzeltildi. **Ancak profil kaydetme hatası bundan kaynaklanmıyordu** — ilk teşhis yanlış çıktı, gerçek sebep yukarıdaki kolon yetkisiydi (yeni build'de de devam etti).
  - **KURAL: Veritabanını sıkılaştıran bir migration uygulamadan ÖNCE, uyarlanmış kodu içeren build'in mağazalarda/TestFlight'ta olduğundan emin ol.** Sıra tersse eski sürümdeki tüm kullanıcılar kırılır ve geri alamazsın (güncellemeyi kullanıcı yapar).
  - Alternatif: geçiş dönemi için policy'yi geçici gevşek tut, yeni sürüm yayıldıktan sonra sık.
...
- **🤖 GOOGLE PLAY (2026-08-17)**: Android paketi `com.htapp.halisaha` (iOS'tan FARKLI — iOS'unki Play'de alınmıştı). AAB 1.0.7/#2 (`7bcd6ce`) dahili teste yüklendi, testçi linki dağıtıldı. Gizlilik politikası yayında: https://gamwichess.github.io/htapp-legal/ (kaynak `legal/gizlilik-politikasi.html`, yayın ayrı public repo `Gamwichess/htapp-legal`). iOS 1.0.8/#12 (`87179e5`) TestFlight'ta. Gereksiz `RECORD_AUDIO` izni kaldırıldı.
- **▶️ SONRAKİ OTURUMUN İLK İŞİ: Android bildirimleri (FCM).** Android'de push HİÇ çalışmıyor — `google-services.json` yok, EAS'te FCM anahtarı yok, `getExpoPushTokenAsync()` sessizce başarısız oluyor. Adım adım liste YAPILACAKLAR.md'nin en başında. Ayrıca `send-notification` edge function'ının service role key kullandığı VARSAYILDI, doğrulanmadı.
- **⏳ AÇIK HATA: profil kaydetme.** `20260817120000_fix_profiles_update_grant.sql` yazıldı, uygulanıp doğrulanacak (yeni build gerekmiyor). Takımım'da kullanıcının kendini görememesi de bununla düzelebilir; düzelmezse ayrı teşhis gerekir.
- **Sonra — Faz 1.2–1.3**: ortak bileşenler (`Card`, `Button`, `Chip`, `SectionHeader`, `EmptyState`, `ListRow`, `Badge`, `Sheet`, `Segmented`, `Avatar`, `IconTile`) + emoji yerine `@expo/vector-icons` (zaten kurulu, YENİ BAĞIMLILIK YOK). Görsel şartname hazır (artifact linki yukarıda) — bileşen dili bölümü doğrudan şartname.
